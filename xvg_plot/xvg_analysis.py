...
@dataclass
class XVGMetadata:  # @dsp obj-c7d2f5a8
...
def parse_data_to_dataframe(data_lines: List[str], metadata: XVGMetadata) -> XVGMetadata:
    ...
    if 'potential' in title_lower or 'energ' in title_lower:
        # Use legend labels from XVG file when available
...
def _parse_y_units(y_label: str) -> List[str]:
    """Parse per-column units from y_label like '(kJ/mol), (K), (bar), (kg/m^3)'."""
    return re.findall(r'\(([^)]+)\)', y_label)


def _build_y_titles_map(y_cols: List[str], y_label: str) -> Dict[str, str]:
    """Build column-name -> 'Name (unit)' map from y_label units string."""
    units = _parse_y_units(y_label)
    result: Dict[str, str] = {}
...
def get_plot_width(n_cols: int) -> int:
    """Return plot width in pixels based on number of grid columns."""
    if n_cols <= 1:
        return 1000
    return 1000 + (n_cols - 1) * 400
...
def plot_energy(df: pd.DataFrame, metadata: XVGMetadata, roll_avg: Optional[int] = None) -> go.Figure:
    ...
    # Use all non-Time columns in their original order
    all_y_cols = [col for col in df.columns if col != 'Time']
    ...
    # Build y-axis title map from parsed y_label units
    y_titles_map = _build_y_titles_map(all_y_cols, metadata.y_label)
...
def compare_energy_plots(df1: pd.DataFrame, df2: pd.DataFrame,
    ...
    # Use all non-Time columns present in both dataframes, preserving order from df1
    all_y_cols = [col for col in df1.columns
                  if col != 'Time' and col in df2.columns]

    n_cols = len(all_y_cols)
    if n_cols == 0:
        return plot_xy(df1, meta1, roll_avg)

    # Build y-axis title map from first file's y_label
    y_titles_map = _build_y_titles_map(all_y_cols, meta1.y_label)
...
def compare_multi_energy(
    ...
    # Use all non-Time columns present in ALL dataframes, preserving order from first
    all_y_cols = [col for col in data_list[0][0].columns
                  if col != 'Time'
    ...
    # Build y-axis title map from first file's y_label
    y_titles_map = _build_y_titles_map(all_y_cols, data_list[0][1].y_label)
